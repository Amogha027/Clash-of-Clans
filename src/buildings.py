...
class Cannon(Building):
    def __init__(self, position, V, level):
        ...
        self.V = V
        self.destroyed = False
        self.health = 20 + 30*level
        self.max_health = 20 + 30*level
        self.type = 'cannon'
        self.attack = 4 + level
...
### begin ###
        ...
        for star in stealth_archers:
            if (ctime - star.btime < 10):
                continue
            if (star.position[0] - self.position[0])**2 + (star.position[1] - self.position[1])**2 <= self.attack_radius**2:
                self.isShooting = True
                self.attack_target(star)
                return
### end ###
...
class Wall(Building):
    def __init__(self, position, V):
        ...
        self.V = V
        self.destroyed = False
        self.health = 20 + 40*V.walls_level[0]
        self.max_health = 20 + 40*V.walls_level[0]
        self.attack = 50
        self.type = 'wall'
        self.level = V.walls_level[0]
...
class WizardTower(Building):
    ...
    def scan_for_targets(self, King):
        self.isShooting = False
        troops = healers + barbarians + archers + dragons + balloons
        for troop in troops:
            if (troop.position[0] - self.position[0])**2 + (troop.position[1] - self.position[1])**2 <= self.attack_radius**2:
                self.isShooting = True
