...
class Healer:
    ...
    def help_troop(self, target, isKing):
        if(self.alive == False):
            return
        
        if isKing == 1:
            target.deal_repair(self.heal)
...
### begin ###
def move_stealth_archers(V, type, King):
    if(type == 1):
        for stealth_archer in stealth_archers:
            ...
            if(stealth_archer.target == None):
                continue
            stealth_archer.move(stealth_archer.target, V,type, King)
    elif(type == 2):
        for stealth_archer in stealth_archers:
            if(stealth_archer.alive == False):
                continue
            ...
            if(closest_building == None):
                continue
            stealth_archer.move(closest_building, V, type, King)
